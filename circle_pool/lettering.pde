...
float[] x_top_down_left = lerpc(0,0,5);
float[] x_top_down_middle = lerpc(0.5,0.5,5);
float[] x_top_down_right = lerpc(1,1,5);
float[] y_top_down = lerpc(0,1,5);

float[] x_td_half_right = lerpc(1,1,3);
float[] x_td_half_left = lerpc(0,0,3);
float[] y_td_half = lerpc(0,0.66,3);
...
float[] xa_full_diagonal = lerpc(0,1,6); //for both
float[] ya_full_diagonal = lerpc(1,0,6); //for both
float[] x_double_diagonal = lerpc(0,1,12); //for both
...
// D
float[] x_dc = {1-1, 1-0.636364, 1-0.266667, 1-0.030303, 1-0.0666667, 1-0.248485, 1-0.484848, 1-0.830303, 1-1};
...
// G
float[] x_gg = {0.654545, 0.981818, 0.939394, 0.781818, 0.381818, 0.0727273, 0.0363636, 0.151515, 0.363636, 0.660606, 0.987879};
float[] y_gg = {0.635897, 0.633333, 0.817949, 0.953846, 0.984615, 0.864103, 0.674359, 0.482051, 0.294872, 0.133333, 0.00769231};
// H
...
// 1
float[] x_11 = concat(append(x_top_down_middle, 0.25),x_dash_short);
float[] y_11 = concat(append(y_top_down, 0.1),y_bottom_2);
//8
float[] x_88 = get8x();
float[] y_88 = get8y();
...
float[] get8x() {
  ...
  for (int i=0; i<dotsy; i++){
    float x = (cos(TAU*i*1.0/dotsy)*0.8+1)*0.5;
    res = append(res, x);
  }
  for (int i=0; i<dotsy; i++){
    float x = (cos(TAU*i*1.0/dotsy)+1)*0.5;
    res = append(res, x);
  }
  return res;
}
...
float[] get8y() {
  ...
  float[] res = {};
  for (int i=0; i<dotsy; i++){
    float y = (sin(TAU*i*1.0/dotsy)+1)*0.5 * 0.66;
    res = append(res, y);
  }
  for (int i=0; i<dotsy; i++){
    float y = (sin(TAU*i*1.0/dotsy)+1)*0.5*0.33+0.66;
    res = append(res, y);
  }
  return res;
}
...
float[][] letter_1 = {x_11, y_11};
float[][] letter_8 = {x_88, y_88};
float[][] letter_space = {{},{}};
...
float[][] getLetter(char letter){
  if(letter == '1') return letter_1;
  if(letter == '8') return letter_8;
...
float getKerning(char prev, char curr){
  if (prev == ' ' || curr == ' ' || prev == '1' || prev == '8' || curr == '1' || curr == '8') return 1.5;
...
float[][] xy_string = getDotString("181818");
